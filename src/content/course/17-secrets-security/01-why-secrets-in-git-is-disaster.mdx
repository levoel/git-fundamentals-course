...
<DiagramContainer title="Lifecycle утечки AWS-ключа">
  <SequenceDiagram>
    <Actor name="DE" />
    <Actor name="GitHub" />
    <Actor name="Bot scanner" />
    <Actor name="AWS" />
    <Lifeline />
    <Message from="DE" to="GitHub" label="git push (ключ в dag.py)" />
    <Message from="GitHub" to="Bot scanner" label="Events API: новый push" />
    <Message from="Bot scanner" to="GitHub" label="GET raw blob" />
    <Message from="Bot scanner" to="Bot scanner" label="regex match: AKIA..." />
    <Message from="Bot scanner" to="AWS" label="aws sts get-caller-identity" />
    <Message from="AWS" to="Bot scanner" label="200 OK — ключ валиден" />
    <Message from="Bot scanner" to="AWS" label="запуск 50 EC2 p3.16xlarge" />
    <Message from="AWS" to="DE" label="Spend alert $11k (через 1 час)" />
  </SequenceDiagram>
</DiagramContainer>
