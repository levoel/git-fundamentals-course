...
<DiagramContainer title="Что происходит после push с AWS ключом в public репо">
  <SequenceDiagram>
    <Actor name="DE" />
    <Actor name="GitHub" />
    <Actor name="GitHub Secret Scanner" />
    <Actor name="AWS Security" />
    <Lifeline />
    <Message from="DE" to="GitHub" label="git push (AKIA... в коде)" />
    <Message from="GitHub" to="GitHub Secret Scanner" label="новый push event" />
    <Message from="GitHub Secret Scanner" to="GitHub Secret Scanner" label="regex match: AKIA..." />
    <Message from="GitHub Secret Scanner" to="AWS Security" label="POST secure channel: leaked key" />
    <Message from="AWS Security" to="AWS Security" label="auto-quarantine ключ" />
    <Message from="AWS Security" to="DE" label="Email: ключ заблокирован" />
    <Message from="GitHub Secret Scanner" to="DE" label="Security alert в репо" />
  </SequenceDiagram>
</DiagramContainer>
